...
pub fn init(width: i32, height: i32) -> ~render::Scene
{
    ...
    else
    {
        ...
        let frag_shdr = render::attach_shader_from_file(pgrm, gl::FRAGMENT_SHADER, render::shader_path(~"unit.fs"));
        let vert_shdr = render::attach_shader_from_file(pgrm, gl::VERTEX_SHADER, render::shader_path(~"unit.vs"));

        gl::bind_frag_data_location(pgrm, 0, ~"outColor");

        match render::link_program(pgrm)
        {
            Ok(pgrm) => {
                gl::use_program(pgrm);
